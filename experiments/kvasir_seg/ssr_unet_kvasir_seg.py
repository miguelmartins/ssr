...
from utils.config.parser import ExperimentConfigParser
from sklearn.model_selection import KFold

CONFIG_FILE = '/home/miguelmartins/Projects/FractalFCN/config/kvasir-seg_baseline.yaml'
LOG_DIR = '/home/miguelmartins/Projects/FractalFCN/logs_baseline/kvasir-seg/train_only/ten-fold'

NUM_FOLDS = 10


def normalize_fn(x, y, minimum_value=0., maximum_value=255.):
    # TODO: esclarecer com o Francesco se o max devia ser por sample, ou devemos assumir que 255
...
def main():
    ...
    for fold, (train_index, val_index) in enumerate(kf.split(dataset_np_x)):
        ...
        fold_data = ExperimentConfigParser(
            name=f'fast-ols-alpha-se-encoders-unet-{NUM_FOLDS}-folds-{fold}-kvasir-seg_pid{os.getpid()}',
            config_path=CONFIG_FILE,
            log_dir=LOG_DIR)
